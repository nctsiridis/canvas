...
AppData default_app_data() {
	...
	ContextComponent default_view = {
		.visible = false, 
		.update=canvas_update, 
		.rect=(RelativeRect){0, 0, 1, 1}, 
		.data=NULL
	};
...
void context_update_all(AppData* app_data) {
	...
	while (context_iter) {
		...
		while (context_component_iter) {
			context_component_iter->context_component.visible = 
				(context_iter->context.index == app_data->context_index) ? true : false;
			context_component_iter->context_component.update(
				app_data, 
				&context_component_iter->context_component
			);
